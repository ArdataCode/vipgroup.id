<?php
namespace PrimeSlider\Modules\Coddle\Widgets;
...
class Coddle extends Widget_Base {
	...
	protected function register_controls() {
		...
		$this->add_responsive_control(
			...
			[
				...
				'selectors'   => [
					'{{WRAPPER}} .bdt-coddle-slider .bdt-coddle-mt-wrap' => 'height: {{SIZE}}{{UNIT}};',
				],
				'render_type' => 'template'
			]
		);
		...
		$this->add_responsive_control(
			...
			[
				...
				'selectors'   => [
					'{{WRAPPER}} .bdt-coddle-slider .bdt-coddle-mt-wrap' => 'max-width: {{SIZE}}{{UNIT}};',
				],
				'render_type' => 'template'
			]
		);
		...
		$this->add_group_control(
			...
			[
				...
				'exclude' => [ 'image' ],
				'selector' => '{{WRAPPER}} .bdt-coddle-slider .bdt-image-wrap:before',
				'fields_options' => [
					'background' => [
						'default' => 'classic',
					],
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .bdt-coddle-slider .bdt-image-wrap:before' => 'mix-blend-mode: {{VALUE}};'
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .bdt-coddle-slider:before' => 'background-image: radial-gradient(circle at 1px 1px, {{VALUE}} 1px, transparent 0);',
				],
				'separator' => 'before'
			]
		);
		...
		$this->add_control(
			...
			[
				'label'     => esc_html__( 'Active Background', 'bdthemes-prime-slider' ),
				'type'      => Controls_Manager::COLOR,
				'selectors' => [
					...
					<div class="swiper-slide bdt-item">
					  ...
					  <div class="bdt-singleTitle-content">
							<?php if ($slide['title'] && ('yes' == $settings['thumbs_show_title'])) : ?>
								<<?php echo Utils::get_valid_html_tag($settings['title_html_tag']); ?> class="bdt-single-title">
	...
	public function render_text($slide) {
		...
		<?php if ($slide['text'] && ('yes' == $settings['show_text'])) : ?>
			<div class="bdt-text <?php echo $text_hide_on_setup; ?>" data-swiper-parallax-y="-70" data-swiper-parallax-duration="800">
	...
	public function render_sub_title($slide) {
		...
		?>
		<h4 class="bdt-sub-title" data-swiper-parallax-y="-70" data-swiper-parallax-duration="600">
	...
	protected function render_play_button($slide) {
		...
		?>
		<div <?php $this->print_render_attribute_string('lightbox'); ?> data-swiper-parallax-y="-70" data-swiper-parallax-duration="700">
		...
		foreach ($settings['slides'] as $slide) : 
			...
			<div class="bdt-item swiper-slide">
				...
				<?php if ($slide['title'] && ('yes' == $settings['show_title'])) : ?>
					<<?php echo Utils::get_valid_html_tag($settings['title_html_tag']); ?> class="bdt-title" data-swiper-parallax-y="-70" data-swiper-parallax-duration="700">
						<?php if ('' !== $slide['title_link']['url']) : ?>
							<a href="<?php echo esc_url($slide['title_link']['url']); ?>">
							<?php endif; ?>
							<?php echo prime_slider_first_word($slide['title']); ?>
